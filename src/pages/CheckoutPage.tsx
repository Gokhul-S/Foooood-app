...
const availableCoupons = [
  { code: 'NEW50', discount: 50, type: 'percentage', description: '50% off on food price', minOrder: 0 },
];

const CheckoutPage = () => {
  const navigate = useNavigate();
  const { items, restaurantName, getTotalPrice, getDeliveryFee, getTaxes, getGrandTotal, clearCart } = useCart();
  ...
  const getFinalTotal = () => {
    return getGrandTotal() - getCouponDiscount();
  };
  ...
  const handleVerifyOtp = async () => {
    ...
    // Simulate OTP verification (any 6-digit OTP is valid)
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          <Label htmlFor={addr.id} className="font-semibold text-foreground cursor-pointer">
                            {addr.type}
                          </Label>
                          {addr.isDefault && (
            ...
            <Card className="p-4">
              ...
              {showCouponInput && !appliedCoupon && (
                <div className="mt-4 flex gap-2">
                  ...
                  />
                  <Button onClick={handleApplyCoupon} className="rounded-xl">
                    Apply
                  </Button>
              ...
              {!appliedCoupon && (
                <div className="mt-4 p-3 bg-secondary rounded-xl">
                  <p className="text-sm text-muted-foreground">
                    Available: <span className="font-semibold text-primary">NEW50</span> - Get 50% off on food!
                  </p>
                </div>
              )}
            </Card>
            ...
            <Card className="sticky top-24 p-4">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className={getDeliveryFee() === 0 ? 'text-foooood-green' : 'text-foreground'}>
                    {getDeliveryFee() === 0 ? 'FREE' : `₹${getDeliveryFee()}`}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">GST & Charges</span>
                  <span className="text-foreground">₹{getTaxes()}</span>
                </div>
                {appliedCoupon && (
                  <div className="flex justify-between text-sm">
